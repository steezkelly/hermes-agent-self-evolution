...
def skill_fitness_metric(
    ...
    if _embed_scorer is not None and _embed_scorer._fitted:
        ...
        # Fallback 1: sentence embedding on-the-fly with just these two texts.
        # This handles the case where the scorer globals were lost in a
        # subprocess or thread (e.g., GEPA's threaded Evaluate).
        score = _on_the_fly_embed_score(agent_output, expected)
        if score is None:
            # Fallback 2: keyword overlap (last resort, noisy but cheap)
            score = 0.5
            expected_lower = expected.lower()
            output_lower = agent_output.lower()
            expected_words = set(expected_lower.split())
