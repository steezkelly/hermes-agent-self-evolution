...
Two separate interception mechanisms exist in the companion system. Know the difference:

#### A. Event Logging — `delegation_intercept.py` (ACTIVE)
...
**Must be explicitly installed.** The patch is NOT applied by default:
```bash
cd ~/hermes0/companion-system && python3 scripts/delegation_intercept.py
```

The patch can be lost on Hermes Agent upgrades (it re-writes `delegate_tool.py`). Check after any `hermes update`:
```bash
grep -c '# -- companion-system delegation interception --' ~/.hermes/hermes-agent/tools/delegate_tool.py
```

#### B. Task Rerouting — `interception_point_inject.py` (SAFE, disarmed)

`scripts/interception_point_inject.py` injects a different interception point that can *reroute* delegation tasks to `data/reroute-queue.jsonl` instead of executing them, intended for future offline companion dispatch.

**SAFETY CRITICAL:** As of 2026-04-30, `pre_delegation_check()` returns `False`. It previously returned `True`, which would have silently hijacked ALL `delegate_task()` calls if the wiring was ever completed. The `hasattr` guard in the injected code prevents activation until a real policy is written.

**The two mechanisms are distinct:**
- `delegation_intercept.py` logs events to `delegation-event-log.jsonl` (observability)
- `interception_point_inject.py` can reroute tasks to `reroute-queue.jsonl` (deferred dispatch)

They can coexist in `delegate_tool.py` without conflict because their call sites are at different locations (logging at the function preamble, rerouting between the `finally` block and the `if n_tasks == 1` guard).

**Verification — check which patches are applied:**
```bash
grep -c '# -- companion-system delegation interception --' ~/.hermes/hermes-agent/tools/delegate_tool.py
grep -c '# -- interception point (injected by interception_point_inject.py) --' ~/.hermes/hermes-agent/tools/delegate_tool.py
```
(expect 1 hit each if both are installed)
...
The companion system's visual dashboard lives at `~/wiki/dashboard.html`. It is a self-contained HTML file with embedded ECharts charts — no external JS dependencies.

**The `file://` CORS problem:** Browsers block `fetch()` from `file://` URLs for local JSON files. The solution is **inline data injection** — the Python refresh script overwrites a `const EMBEDDED_DASHBOARD_DATA = "..."` string inside the HTML directly, so the page never makes an HTTP request.

Cron job `1f8090293f4f` runs `~/.hermes/scripts/update-dashboard-and-today.py` every 5 minutes to:
1. Query Mnemosyne stats
2. Overwrite `dashboard-data.json` (raw data backup)
3. Inject a minified JSON string into `dashboard.html` via regex replacement of `EMBEDDED_DASHBOARD_DATA`
4. Refresh `today.md` (the markdown daily brief)

If the dashboard appears blank in Firefox, the user likely needs to **refresh the page** (F5) — the cron has already run with the new logic, the browser is showing a stale cached state.
...
- `~/.hermes/scripts/update-dashboard-and-today.py` — Dual refresh: dashboard HTML data injection + today.md brief
- `~/wiki/dashboard.html` — Self-contained visual dashboard (ECharts, inline data)
- `~/wiki/today.md` — Canonical daily brief (markdown)
